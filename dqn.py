...
def dqn_learning(env,
    ...
    model_resnet_boss.load_state_dict(torch.load(
        'D:/RL/Black-Myth-Wukong-AI/dataset/boss_model.pkl'))
    ...
    # load checkpoint
    if checkpoint != 0:
        checkpoint_path = "models/wukong_guangzhi_" + str(checkpoint) + ".pth"
    ...
    for t in itertools.count(start=checkpoint):
        ...
        output_boss, intermediate_results_boss = model_resnet_boss(obs.to(device))
        ...
        self_power_window = (1557,971,1604,1031) # 棍势点
        ...
        self_endurance_window = (184,997,500,1005) # 耐力条
        ...
        ding_shen_window = (1455, 832, 1485, 848)
        ...
        if ding_shen_available == True: # 定身好了就定身
        ...
        # print(state_list)
        if action == 4 and self_power < 30:  # 把重棍处理成三连棍
            action = 2
        # elif action == 5 and self_endurance > 200: # 把歇脚回气力处理成轻棍
        #     action = 0
        
        obs, reward, done, stop, emergence_break = env.step(
            action, boss_attack, self_power)
        ...
        if t % SAVE_MODEL_EVERY_N_STEPS == 0:
            ...
            model_save_path = "models/wukong_guangzhi_%d.pth" % (t)
